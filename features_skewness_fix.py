# -*- coding: utf-8 -*-
"""
Created on Tue Apr 16 14:06:40 2019
...
def secDiffMean(a):
    ...
    output = zeros(len(b))  # Initializing the output array with zeros (length = 14)
    temp1 = zeros(len(b[0]) - 1)  # To store the 1st Diffs
    k = 0;  # For counting the current row no.
    for i in b:
        t = 0.0
...
def skewness(arr):
   ...
   # for i in data:
    skew_array = sp.stats.skew(data, axis=0, bias=True)
